...
# ===========================
    ...
    # Create tabs for dataset info
    tab_about, tab_tracking = st.tabs(["📚 About the Dataset", "🌡️ What We're Tracking"])

    with tab_about:
        st.markdown('<div class="section-header">📚 Where This Data Comes From</div>', unsafe_allow_html=True)

        st.markdown("""
        **Dataset Source:**
        - **Origin**: [Climate Change Indicators on Kaggle](https://www.kaggle.com/datasets/tarunrm09/climate-change-indicators)
        - **Timespan**: 1961 to 2022 (62 years of continuous data)
        - **Geographic Coverage**: 195 countries worldwide
        - **Data Type**: Annual climate and emissions indicators

        **What Makes This Dataset Valuable:**

        This dataset brings together multiple authoritative sources into one comprehensive climate database:

        **Primary Data Sources:**
        - **Temperature Data**: Collected from meteorological stations worldwide, compiled by climate research institutions
        - **Emission Records**: National emissions reports submitted to international climate bodies
        - **Sea Level Data**: Satellite measurements and tide gauge records from coastal monitoring stations
        - **Deforestation Rates**: Satellite imagery analysis tracking forest cover changes
        - **Methane Measurements**: Atmospheric monitoring from global observation networks

        **Data Collection Methods:**
        - Ground-based weather stations with standardized instruments
        - Satellite remote sensing for global coverage
        - National inventory reports verified by international agencies
        - Scientific research institutions conducting long-term monitoring

        **Quality & Reliability:**
        - ✅ Cross-validated across multiple independent sources
        - ✅ Peer-reviewed methodologies for data collection
        - ✅ Regular quality control and error correction
        - ✅ Transparent documentation of measurement techniques
        - ✅ Standardized metrics allowing global comparison
        ...
        **Key Indicators Included:**
        ...
        with col1:
            ...
            **Climate Impact Measurements:**
            - 🌡️ **Temperature Change**: Annual average temperature anomaly compared to historical baseline
            - 🌊 **Sea Level Rise**: Millimeters of sea level change per year
            - 📉 **Extreme Weather Frequency**: Count of record-breaking temperature events
            """)

        with col2:
            st.markdown("""
            **Human Activity Indicators:**
            - 💨 **CO₂ Emissions**: Millions of metric tons per country per year
            - 🔥 **Methane Emissions**: Kilotons of CH₄ released
            - 🌳 **Deforestation Rate**: Square kilometers of forest lost annually
            """)
        ...
        **Dataset Structure:**
        ...
        The data is organized as a time-series with the following structure:
        - **Rows**: Each row represents one country in one year
        - **Total Records**: ~12,090 observations (195 countries × 62 years)
        - **Columns**: Key indicators plus geographic and temporal identifiers

        **Sample Data Structure:**
        """)

        # Create a sample dataframe to show structure
        sample_data = {
            'Year': [1961, 1962, 2021, 2022],
            'Country': ['USA', 'USA', 'China', 'China'],
            'Temperature_Change': [0.03, 0.05, 1.15, 1.20],
            'CO2_Emissions': [2500, 2600, 11000, 11500],
            'Sea_Level_mm': [0, 2, 95, 98]
        }
        st.dataframe(pd.DataFrame(sample_data), use_container_width=True)
        st.caption("Simplified example showing dataset structure (actual dataset includes more variables)")

        st.markdown("---")

        st.markdown("""
        **Why This Dataset Matters for Analysis:**

        **Long Time Horizon (62 years):**
        - Captures both slow trends and sudden shifts
        - Allows us to distinguish climate patterns from natural variability
        - Shows acceleration of change over decades

        **Global Coverage (195 countries):**
        - Reveals regional differences in climate impact
        - Identifies highest emitters vs. most vulnerable nations
        - Enables country-level policy recommendations

        **Multiple Indicators:**
        - Shows relationships between causes (emissions) and effects (temperature)
        - Reveals which factors most strongly predict warming
        - Helps identify effective intervention points

        **Consistent Methodology:**
        - Standardized measurements across countries and years
        - Enables valid statistical comparisons
        - Reliable foundation for predictive modeling
        """)

        st.info("""
        **Data Integrity Note**: All visualizations and analyses in this application are derived directly
        from this Kaggle dataset without modification to the underlying measurements. Any transformations
        (like calculating averages or growth rates) are documented in the project notebooks.
        """)

    with tab_tracking:
        # Original "What we track" content
